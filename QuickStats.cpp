...
int QuickStats::average(int samples[],int m)
{
  int total1=0.0;
  for(int i=0;i<m;i++){
...
int QuickStats::minimum(int samples[],int m)
{
...
int QuickStats::maximum(int samples[],int m)
{
  int sorted[m];   //Define and initialize sorted array
  for(int i=0;i<m;i++){
    sorted[i]=samples[i];
...
int QuickStats::stderror(int samples[],int m) {
  int temp1=0;
  temp1=stdev(samples,m);
  return (temp1/sqrt(m));
}

int QuickStats::CV(int samples[],int m)  //Coefficient of variation (%RSD, or relative stdev)
{
  int avg=0;
  int sd=0;
  avg=average(samples,m);
...
void QuickStats::bubbleSort(int A[],int len) {
  unsigned long newn;
  unsigned long n=len;
  int temp=0;
  do {
...
int QuickStats::fabs(int sample) // calculate absolute value
{
  if(sample<0){
    return -sample;
  }else{
...
int QuickStats::mode(int samples[],int m,int epsilon) //calculate the mode.
